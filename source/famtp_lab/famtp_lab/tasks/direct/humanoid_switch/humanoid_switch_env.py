"""DirectRLEnv implementation for humanoid skill switching with FaMTP stage-1 priors."""
...
class HumanoidSwitchEnv(DirectRLEnv):
    """Env with ppo_cmd/fullbody_amp/partwise_raw/famtp_stage1 prior modes."""
    ...
    def _compute_stage1_latents(self) -> tuple[dict[str, torch.Tensor], torch.Tensor]:
        """Compute part latents and packed summary.

        Returns:
            latent_out: dict containing z_by_part/theta_by_part/z_concat
            latent_summary: (N, 5*(2+D_res))
        """
        latent_out = self._manifold_encoder(self.part_history)
        latent_summary = latent_out["z_concat"]
        return latent_out, latent_summary
    ...
    def _apply_action(self) -> None:
        ...
        # Update part history used by latent encoders.
        _ = self._update_part_history()
    ...
    def _get_observations(self) -> dict[str, torch.Tensor]:
        latent_summary = None
        if self.cfg.prior_mode == "famtp_stage1" and self.cfg.use_manifold_encoder:
            latent_out, latent_summary = self._compute_stage1_latents()
            self.extras["latent/z_concat"] = latent_out["z_concat"].clone()
        else:
            # Keep observation dimension stable across modes.
            latent_summary = torch.zeros(
                (self.num_envs, len(PART_NAMES) * (2 + self.cfg.latent_dim_residual)),
                device=self.device,
            )
        obs = build_policy_obs(self.state, self.current_skill_id, self.target_skill_id, latent_summary=latent_summary)
        return {"policy": obs}

    def _famtp_stage1_terms(self) -> dict[str, torch.Tensor]:
        """Compute FaMTP stage-1 prior terms."""
        terms: dict[str, torch.Tensor] = {}
        if not self.cfg.use_manifold_encoder:
            return terms

        latent_out, _ = self._compute_stage1_latents()
        ...
        if self.cfg.use_global_coupling:
            # Synthetic whole-body context features from current state.
            # root_vel/com_feat/ang_mom_proxy: (N, 3), contact_flags: (N, 4)
            ...
            with torch.no_grad():
                coupling_logits = self._coupling_scorer(
                    latent_out["z_concat"], root_vel=root_vel, com_feat=com_feat, contact_flags=contact_flags, ang_mom_proxy=ang_mom_proxy
                )
            terms["latent_global_coupling"] = logits_to_imitation_reward(coupling_logits, scale=self.cfg.rew_global_coupling_scale)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        elif self.cfg.prior_mode == "famtp_stage1":
            reward_terms.update(self._famtp_stage1_terms())
        elif self.cfg.prior_mode == "famtp_full":
            # Stage 2 bridge generation attaches here (bridge generator + transition priors).
            pass
        else:
